...
window.payNow = async function (bookingId, amount) {
    try {
        ...
        const res = await fetch("http://localhost:3000/api/payments/razorpay/order", {
            method: "POST",
            headers: {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    async function loadPaymentPage(bookingId) {
        try {
            ...
            // Update UI
            document.getElementById('mentor-name-summary').textContent = booking.mentorName;
            document.getElementById('mentor-img-summary').src = booking.mentorPic || "assets/images/default-avatar.png";
            document.getElementById('mentor-rating-summary').textContent = booking.mentorRating.toFixed(1);
            document.getElementById('mentor-skill-summary').textContent = `Mentorship on: ${booking.topic}`;
            
            const duration = booking.duration || 10; // Defaulting to 10 mins as requested
            document.getElementById('session-duration').textContent = `${duration} mins`; 
            ...
            if (booking.date) {
                ...
                if (!isNaN(dateObj)) {
                    displayDate = dateObj.toLocaleDateString('en-US', { weekday: 'short', year: 'numeric', month: 'short', day: 'numeric' });
                }
            }
            document.getElementById('session-datetime').textContent = `${displayDate} at ${booking.time}`;
            ...
            document.getElementById('session-price').textContent = `₹${mentorAmount.toFixed(2)}`;
            ...
            document.getElementById('pay-amount-final').textContent = finalPrice.toFixed(2);
